...
async def _run_parse_pipeline(user_id: int, paper_id: int, pdf_key: str, task_id: int) -> None:
    ...
    await mysql.execute(
        "UPDATE ingest_tasks SET stage='parsing', progress=10, started_at=%s WHERE id=%s",
        datetime.now(), task_id,
    )

    try:
        from services.parsing.parser import parse_paper
        parse_result = await parse_paper(user_id, paper_id, pdf_key)
    except Exception:
        await mysql.execute(
            "UPDATE ingest_tasks SET stage='failed', error_msg=%s WHERE id=%s",
    ...
    # --- Stage 2: Indexing ---
    await mysql.execute(
        "UPDATE ingest_tasks SET stage='indexing', progress=50 WHERE id=%s",
        task_id,
    )

    try:
        from services.indexing.indexer import index_paper
        await index_paper(parse_result, user_id, paper_id)
    except Exception:
        await mysql.execute(
            "UPDATE ingest_tasks SET stage='failed', error_msg=%s WHERE id=%s",
            traceback.format_exc(), task_id,
    ...
    # --- Done ---
    await mysql.execute(
        "UPDATE ingest_tasks SET stage='done', progress=100, finished_at=%s WHERE id=%s",
        datetime.now(), task_id,
    )
    await mysql.execute(
    ...
    await mysql.execute(
        "UPDATE ingest_batches SET done=done+1 WHERE id=(SELECT batch_id FROM ingest_tasks WHERE id=%s)",
        task_id,
    )
